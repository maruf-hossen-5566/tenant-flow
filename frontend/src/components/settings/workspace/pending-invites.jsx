...
import {deleteInvitation, getPendingInvitations} from "@/api/invitation-api.js";
...
import {Item, ItemContent, ItemGroup, ItemMedia, ItemSeparator, ItemTitle} from "@/components/ui/item.jsx";
...
const PendingInvites = () => {
    ...
    useEffect(() => {
        const fetchInvitations = async () => {
            try {
                ...
                toast.error(e?.response?.data?.detail || "Failed to fetch members")
            }
        }
        fetchInvitations()
    }, [searchQuery, skip]);
    ...
    const handleDeleteInvite = async (iId) => {
        ...
        try {
            ...
            setData({...data, ["items"]: newMembers, ["total"]: data?.total - 1})
            setSearchParams(() => {
                const newParams = new URLSearchParams();
    ...
    return (<div className="bg-background w-full flex min-h-svh flex-col gap-6 p-6 md:p-10">
                        ...
                        <ItemGroup className="gap-2">
                            {data?.items?.map((i) => (
                                ...
                                >
                                    <ItemMedia>
                                        <Avatar>
                                            <AvatarFallback className="capitalize">{i?.email?.charAt(0)}</AvatarFallback>
                                        </Avatar>
                                    </ItemMedia>
                                    <ItemContent className="w-full flex flex-row items-center justify-between">
                                        ...
                                        <Badge className="bg-yellow-500/50 text-white capitalize ml-4 mr-auto">Pending</Badge>
                                    </ItemContent>
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button
                                                size="icon"
                                                variant="ghost"
                                                disabled={!canCrud && currentMember?.id !== i?.id}
                                            >
                                                <Ellipsis/>
                                            </Button>
                                        </DropdownMenuTrigger>
