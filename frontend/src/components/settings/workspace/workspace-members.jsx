...
import {changeMemberRole, getActiveMembers, removeMember} from "@/api/membership-api.js";
...
const WorkspaceMembers = () => {
    ...
    const setActiveMembers = useWorkspaceStore(state => state.setActiveMembers)
    ...
    const handleRemoveMember = async (mId) => {
        ...
        try {
            ...
            setActiveMembers((newMembers && newMembers?.length > 0) ? newMembers : null)
            setSearchParams(() => {
    ...
    const handleMemberRoleChange = async (value, memberId) => {
        try {
            ...
            const newMembers = activeMembers?.map(m => m?.id === memberId ? res?.data : m)
            setActiveMembers((newMembers && newMembers?.length > 0) ? newMembers : null)
            ...
            console.log("E: ", e)
            toast.error(e?.response?.data?.detail || "Failed to change the role")
        }
    }
                                ...
                                >
                                    ...
                                    <ItemContent className="w-full flex flex-row items-center justify-between">
                                        ...
                                        <div className="flex items-center justify-end gap-4">
                                            <Select
                                                value={m?.role}
                                                onValueChange={(value) => handleMemberRoleChange(value, m?.id)}
                                                disabled={!canCrud}
                                            >
                                                <SelectTrigger
