...
import {Item, ItemActions, ItemContent, ItemGroup, ItemSeparator, ItemTitle} from "@/components/ui/item.jsx";
...
import {fetchAndSetMembers, fetchAndSetProjects} from "@/lib/fetch-and-set-methods.js";
...
const ProjectsComp = ({data, setData}) => {
    ...
    const handleProjectJoin = async (e, projectId) => {
        ...
        try {
            ...
            const newProjects = data?.items?.map(p => p?.id === res?.data?.id ? res?.data : p)
            setData({...data, ["items"]: newProjects})
        } catch (e) {
            console.log(e)
            toast.error(e?.response?.data?.detail || "Failed to join the project")
            return
        }
        ...
        <div className="bg-background w-full flex min-h-svh flex-col gap-6 p-6 md:p-10">
            <div className="w-full max-w-3xl mx-auto flex flex-col">
                <h1 className="py-12 text-xl flex items-center gap-2 font-bold">
                    Projects
                    {data &&
                        <div className="py-1 px-3 rounded-full bg-accent">{data?.total || 0}</div>}
                </h1>
                                        ...
                                        >
                                            <Link to={`./${project?.id}`}>
                                                <ItemContent className="w-full flex flex-row items-center justify-between gap-12">
                                                    <ItemTitle className="max-w-sm line-clamp-1">{project?.name}</ItemTitle>
                                                    {
                                                        project?.members && (
                                                            <div
                                                                className="ml-auto mr-4 items-center justify-end *:data-[slot=avatar]:ring-background flex -space-x-2 *:data-[slot=avatar]:ring-2 *:data-[slot=avatar]:grayscale"
                                                            >
                                                                {
                                                ...
                                                <ItemActions>
                                                    <Button
                                                        size="sm"
                                                        variant={
                                                            !project?.members?.some(_m => _m?.user_id === user?.id)
                                                                ? "outline" :
                                                                "destructive"
                                                        }
                                                        onClick={(e) => handleProjectJoin(e, project?.id)}
                                                    >
                                                        {
                                                            !project.members.some(_m => _m.user_id === user.id)
                                                                ? "Join" :
