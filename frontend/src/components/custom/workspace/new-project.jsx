...
const NewProject = ({data, setData}) => {
    ...
    const setActiveProjects = useWorkspaceStore((state) => state.setActiveProjects);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const {sd, dd} = validateStartAndEndDate(cleanedDate?.startDate, cleanedDate?.dueDate)
        ...
        try {
            ...
            setActiveProjects(activeProjects ? [res?.data, ...activeProjects] : [res?.data])
            const newData = {...data}
    ...
    const handleMemberAdd = (id) => {
        ...
        if (memberExists) {
            setDraftProject("members", oldMembers?.filter(i => i !== id))
        } else {
    ...
    return (<SheetContent className="pb-12 overflow-y-auto">
                        ...
                        <FieldGroup>
                            <Field>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setDraftProject("name", e?.target?.value)}
                                    required
                                />
                            </Field>
                            <Field>
                                ...
                                <Textarea
                                    ...
                                    onChange={(e) => setDraftProject("desc", e?.target?.value)}
                                />
                            </Field>
                            <Field>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => setDraftProject("lead", value || null)}
                                >
                                    <SelectTrigger id="project-lead-ts6">
                                        <SelectValue placeholder="--"/>
                                    </SelectTrigger>
                                    {members && <SelectContent>
                                ...
                                <FieldDescription className="text-xs">Lead and Creator are members by
                            ...
                            <Field>
                                ...
                                <DropdownMenu
                                    className="max-w-xs min-w-xs w-full"
                                >
                                    <DropdownMenuTrigger
                                    ...
                                    >
                                        ...
                                        >
                                            {draftProject?.members?.length > 0 ?
                                                <span>{draftProject?.members?.length}{" "}{pluralize("member", draftProject?.members?.length)}</span>
                                                :
                                                <span>--</span>
                                            }
                                            <ChevronDown/>
                                    ...
                                    {members && <DropdownMenuContent className="max-w-xs min-w-xs w-full">
                                        {members?.map((m) => (<DropdownMenuCheckboxItem
                                            key={m?.id}
                                            checked={draftProject?.members?.includes(m?.id)}
                                            onCheckedChange={() => handleMemberAdd(m?.id)}
                                        >{m?.user?.email}</DropdownMenuCheckboxItem>))}
                                    </DropdownMenuContent>}
                                </DropdownMenu>
                            </Field>
                            <Field>
                                ...
                                >
                                    <PopoverTrigger asChild>
                                        ...
                                        >
                                            {draftProject?.startDate ? moment(draftProject?.startDate).format("MMM DD, YYYY") : "--"}
                                            <ChevronDownIcon/>
                                        </Button>
                                    </PopoverTrigger>
                                    <PopoverContent
                                    ...
                                    >
                                        <Calendar
                                            mode="single"
                                            selected={draftProject?.startDate}
                                            captionLayout="dropdown"
                                            onSelect={(date) => {
                                                setDraftProject("startDate", date)
                                ...
                                >
                                    <PopoverTrigger asChild>
                                        <Button
                                            ...
                                            className="w-48 justify-between font-normal text-muted-foreground"
                                        >
                                            {draftProject?.dueDate ? moment(draftProject?.dueDate).format("MMM DD, YYYY") : "--"}
                                            <ChevronDownIcon/>
                                        </Button>
                                    </PopoverTrigger>
                                    <PopoverContent
